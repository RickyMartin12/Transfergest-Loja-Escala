...
/*VERIFICAR MAIOR NUMERO EM nid VALOR DAS ENTRADAS DA API */
$result = mysqli_query($conn, "SELECT nid FROM excel ORDER BY nid DESC LIMIT 1");
$row = mysqli_fetch_array($result);

$db=$row['nid'];

$db <= 2360 ? $nid = 2361  : $nid = $db+1;
		...
		if ($result){
	        ...
	        if ($retorno == 1) {
				...
				if(!empty($out->cliente_transfer->pickupTime)){
                     ...
                     if (strpos ($out->cliente_transfer->pickupTime,'Fatal error:') !== false) {
                         $timeok = strtotime($date_array[2].'-'.$date_array[1].'-'.$date_array[0].' 00:01'); 
                    }
                    else {
                                        $horas = str_replace('h',':',$out->cliente_transfer->pickupTime);
