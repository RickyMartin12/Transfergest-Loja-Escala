...
case '1':
		...
		if ($_POST['coord'] != "") 
      	{
            $_POST['coord'] = filter_var($_POST['coord'], FILTER_SANITIZE_STRING);
        ...
        /*VALIDAR DATA E ATRIBUIR VAR SESSÃO*/
        if ($_POST['voo_che_dt'] != "") 
        {
...
/**/
		/*VALIDAR NOME CLIENTE E ATRIBUIR VAR SESSÃO*/
        if ($_POST['nome'] != "") 
        {
...
/* CAMPOS PASSAGEIROS*/
/**/
			// Validar o Numero de passageiros

			// Validar os Passageiros que sao os adultos
           ...
           if ($_POST['adult'] != "") 
           {
	        ...
	        // Validar os Passageiros que sao as crianças
            ...
            // Validar os Passageiros que sao os bebes
	       ...
	       // Validar os Passageiros que sao as observações
	        ...
	        // Identificar e Validar o Tipo do Cliente que vai colocar
	        if ($_POST['payments'] != "") {
        ...
        if($errors == "")
        {
    		...
    		// Valores de Retorno dos dados da Loja
        	...
        	if ($_SESSION["payments"] =='Driver')
    		{
    			...
    			$data_arrival = $_SESSION["voo_che_dt"];

				$dat_arr=explode(' ',trim($data_arrival));
				$date_array = explode('/', $dat_arr[0]);

                $date=strtotime($date_array[2].'-'.$date_array[1].'-'.$date_array[0].'-00');
                $timeok = strtotime($date_array[2].'-'.$date_array[1].'-'.$date_array[0].' '.$dat_arr[1]);


				$obs = '('. $_SESSION["pax"] .') ' .' | MTR | OBS: '. $_SESSION["obs"]; 
                                $gerado = $_SESSION["dominio"].'@'.date("Y-m-d H:i:s");
                                

				$new_arrival = "INSERT INTO excel(servico,
				data_servico, hrs, voo, nome_cl, email, local_recolha, morada_recolha, local_entrega, morada_entrega, px, cr, bebe, obs,operador,cobrar_direto,data_criacao, referencia, telefone, pais, criado_direto, end, estado, navigator, morada_recolha_gps, morada_entrega_gps, voo_horario, created_by)
				VALUES ( '$servico', $date, $timeok, '{$_SESSION["voo_n_che"]}', '{$_SESSION["nome"]}', '{$_SESSION["email"]}', '{$_SESSION["from"]}', '{$_SESSION["voo_che_mor"]}', '{$_SESSION["to"]}', '{$_SESSION["voo_dep_mor"]}','{$_SESSION["adult"]}', '{$_SESSION["children"]}', '{$_SESSION["baby"]}', '$obs', 'Direto', $val, '".date("Y-m-d H:i:s")."', '$referencia', '{$_SESSION["tel"]}', '{$_SESSION["country"]}','{$_SESSION["dominio"]}', $duration, 'Pendente', '{$_SESSION["navigator"]}', '{$_SESSION["coord"]}', '{$_SESSION["coord2"]}', '-/-', '$gerado')";
    		...
    		if ($_SESSION["payments"] =='Bank')
    		{
				...
				$obs = '('. $_SESSION["pax"] .') ' .' | TB | OBS: '. $_SESSION["obs"]; 

                                $gerado = $_SESSION["dominio"].'@'.date("Y-m-d H:i:s");


				$new_arrival = "INSERT INTO excel(servico,
				data_servico, hrs, voo, nome_cl, email, local_recolha, morada_recolha, local_entrega, morada_entrega, px, cr, bebe, obs,operador,cobrar_operador,data_criacao, referencia, telefone, pais, criado_direto, end, estado, navigator, morada_recolha_gps, morada_entrega_gps, voo_horario, created_by)
				VALUES ( '$servico', $date, $timeok, '{$_SESSION["voo_n_che"]}', '{$_SESSION["nome"]}', '{$_SESSION["email"]}', '{$_SESSION["from"]}', '{$_SESSION["voo_che_mor"]}', '{$_SESSION["to"]}', '{$_SESSION["voo_dep_mor"]}','{$_SESSION["adult"]}', '{$_SESSION["children"]}', '{$_SESSION["baby"]}', '$obs', 'Site Transf.Banco', $val, '".date("Y-m-d H:i:s")."', '$referencia', '{$_SESSION["tel"]}', '{$_SESSION["country"]}','{$_SESSION["dominio"]}', $duration, 'Pendente', '{$_SESSION["navigator"]}','{$_SESSION["coord"]}', '{$_SESSION["coord2"]}', '-/-', '$gerado')";
    		...
    		if ($_SESSION["payments"] =='Paypal') // Leva muito tempo a excutar
    		{
				...
				$new_arrival = "INSERT INTO excel(servico,
				data_servico, hrs, voo, nome_cl, email, local_recolha, morada_recolha, local_entrega, morada_entrega, px, cr, bebe, obs,operador,cobrar_operador,data_criacao, referencia, telefone, pais, criado_direto, end, estado, navigator, morada_recolha_gps, morada_entrega_gps, voo_horario, created_by)
				VALUES ( '$servico', $date, $timeok, '{$_SESSION["voo_n_che"]}', '{$_SESSION["nome"]}', '{$_SESSION["email"]}', '{$_SESSION["from"]}', '{$_SESSION["voo_che_mor"]}', '{$_SESSION["to"]}', '{$_SESSION["voo_dep_mor"]}','{$_SESSION["adult"]}', '{$_SESSION["children"]}', '{$_SESSION["baby"]}', '$obs', 'Site Paypal', $val, '".date("Y-m-d H:i:s")."', '$referencia', '{$_SESSION["tel"]}', '{$_SESSION["country"]}','{$_SESSION["dominio"]}', $duration, 'Pendente', '{$_SESSION["navigator"]}','{$_SESSION["coord"]}', '{$_SESSION["coord2"]}', '-/-', '$gerado')";
		...
		if (mysqli_num_rows($result) > 0) 
		{
		    ...
		    while($row = mysqli_fetch_assoc($result))
		    {
		        ...
		        if (!$cr) $cr=0;
		        if (!$bebe) $bebe=0;
		        if (!$morada_entrega) $morada_entrega='-/-';
		        if (!$morada_recolha) $morada_recolha='-/-';
		        ...
		        else
		        {
		        	...
		        	$acobrar = "-/-";
				        ...
				        $pdf->SetFont('Arial','',9);
				        $pdf->Cell(32,6,utf8_decode($tp_serv),0,0,'L',true);        
				        // Retorno
				        $pdf->SetFont('Arial','B',9);
				        ...
				        // Estado
				        $pdf->SetFont('Arial','B',9);
				        $pdf->Cell(15,6,utf8_decode($lang['STATE']),0,0,'L',true);
				        $pdf->SetFont('Arial','',9);
