...
if (!isset($error_message))
 {
  ...
  $rel_diario = (isset($_POST['rel_diario'])) ? 'checked' : 'false';
  $rel_semana = (isset($_POST['rel_semana'])) ? 'checked' : 'false';
...
$operator_new = "INSERT INTO operador (nome,responsavel,email,tipo,comissao,tel,utilizador,password,gestao,rel_diario,rel_semana)
VALUES ('$nome','{$_POST['operatorresponsavel']}','{$_POST['operatoremail']}','{$_POST['operatortpcomissao']}','{$_POST['operatorcomissao']}','{$_POST['operatortel']}','$utilizador','$pass','$gestao','$rel_diario','$rel_semana')";
...
case '3':
$obter_operator=" SELECT id,nome,responsavel,comissao,email,tipo,utilizador,tel,gestao,rel_diario,rel_semana FROM operador WHERE 1 ORDER BY nome";
...
$query=" SELECT nome FROM operador WHERE nome ='{$_POST['nome']}' LIMIT 2";
	...
	if($rowCount>1) { 
 	  $error_message = 9;
        ...
        echo $rowCount;
	if ( !isset($error_message) ) {
    	  $fields=array('nome','responsavel','comissao','email','tel','tipo','utilizador');
