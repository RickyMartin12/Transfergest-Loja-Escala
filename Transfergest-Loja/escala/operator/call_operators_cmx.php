      ...
      <div class='row'>
        <div class="col-md-4">
          <h3 class="panel-title" style="color:#000!important;">
           <span class="glyphicon glyphicon-tower"></span> Comissões
          </h3>
        </div>
        <div class="col-md-8" style="text-align: right;">
          ...
          <a href="#" class="received btn btn-success disabled" title="Validar Comissão dos serviços como valor Pago ao Operador, campo Comissão Operador"><span class="glyphicon glyphicon-eur"></span><span class="hidden-xs"> Validar</span></a>
        </div>
      </div>
      ...
      <div class='row nosearchprint no-print'>
        <div class='col-sm-4 col-xs-12 staff_date'>
          <div class="form-group input-group">
            ...
            <select type="text" class="form-control" id="staff_name" name="staff_name" placeholder="Nome Operador">
            </select>
          </div>
        </div>
...
function searchValue(vl){
  $("#example, #example_container2, #example_container, .search").css("display","block");
   ...
   "paging": false,
    ...
    "ajax":
      {
        "url" : "operator/action_datatables_cmx_operators.php?"+vl,
        "type": "GET"
...
*/
      ...
      columns: [
        ...
        { data: "id", render: function ( data, type, row ) {
          return '<button title="Validar Cobrança Serviço #'+data+'" class="btn-sm btn-validate btn btn-warning" style="margin-left: 5px;">&nbsp;<i class="fa fa-eur"></i>&nbsp; </span></button><button title="Ver todos os detalhes do Serviço #'+data+'" class="btn-sm btn btn-expand btn-info"><i class="fa fa-exclamation-circle"></i> '+data+'</button>';
          }
},
        { data: "referencia" },
        ...
        { data: "cmx_op", className: "number-right",
         ...
         {data: "pag_ope", className:"hidden payok"}
        ],
     select: 
        {
          style:    'os',
        ...
        buttons: [
          {
            ...
            columns: ':not(:nth-child(6),:nth-child(7),:nth-child(9),:nth-child(10),:nth-child(11),:nth-child(12),:nth-child(13),:nth-child(14),:nth-child(15),:nth-child(16),:nth-child(19),:nth-child(20),:nth-child(23),:nth-child(24),:nth-child(25),:nth-child(27),:nth-child(28),:nth-child(29),:nth-child(30),:nth-child(31),:nth-child(32),:nth-child(33),:nth-child(34),:nth-child(36))'
          },
          {
            extend: 'print', 
          ...
          },
          {
            ...
            action:function(e,dt,button,config){
              if($("#staff_name").val()){
                 file_1 ='operador_'+$("#staff_name").val()+'_';
                 title_1= 'Operador:'+$("#staff_name").val();}
              else {file_1='';title_1 ='';};
              ...
              config.filename ='cobrancas_operador_'+file_1+''+file_3+''+file_4;
...

          {
            ...
            action:function(e,dt,button,config){
              ...
              if($("#date_fim").val()){
                ...
                file_4='';title_4='';}
              config.filename ='comissao_operador_'+file_1+''+file_3+''+file_4;
              config.title ='Comissão Operador: '+title_1+''+title_3+''+title_4;
...
if (data['pag_ope'] !='Sim')
 $(row).find('td:eq(0) .btn-validate').removeClass('btn-success').addClass('btn-warning').attr('title','Validação Comissão Serviço #' +data['id']);
             ...
             "columnDefs": [
               ...
               {
                 "targets": [5,6,8,10,11,12,13,14,15,18,19,22,23,24,26,27,28,29,30,31,32,33],
                 "visible": false,
                 "searchable": false,
                }
             ],
...
$('#example tbody').on( 'click', '.btn-warning', function () {
   ...
   idarr=[];
   idarr.push(arr.id);
   ServicesPayment(idarr,arr.cmx_op,1);
});
...
$('#example tbody').on( 'click', '.btn-success', function () {
   ...
   idarr=[];
   idarr.push(arr.id);
   ServicesPayment(idarr,arr.cmx_op,0);
});
...
if (arr.length >=1){
bootbox.dialog({
  message: "Tem a certeza que pretende <b>"+isvalid+"</b>, "+arr.length+" serviço(s) com ID: <strong>"+txt+"</strong> no Campo Comissão Operador?<br>O valor total é de: "+vl+"</strong>",
  ...
  buttons: {
    ...
    danger: {
      ...
      $.ajax({ 
      url:'operator/action_close_cmx_operators.php',
      ...
      ar = JSON.parse(data);
       if(!ar.success.match(/0/g)){  
         $('.debug-url').html('O(s) '+arr.length+' serviço(s) com ID: <strong>'+txt+'</strong> para estado: '+isvalid+', no campo Comissão Operador.');
         $("#Modalok").modal();
         $('.received').addClass('disabled');
         txt='';
