      ...
      <div class='row'>
        <div class="col-md-4">
          <h3 class="panel-title" style="color:#000!important;">
           <span class="glyphicon glyphicon-tower"></span> Cobranças
          </h3>
        </div>
        <div class="col-md-8" style="text-align: right;">
          ...
          <a href="#" class="received btn btn-success disabled" title="Validar Cobrança dos serviços como valor Recebido do operador, campo Cobrar Operador"><span class="glyphicon glyphicon-eur"></span><span class="hidden-xs"> Validar</span></a>
        </div>
      </div>
...
function searchValue(vl){
  $("#example, #example_container2, #example_container, .search").css("display","block");
   ...
   "paging": false,
    ...
    "ajax":
      {
        "url" : "operator/action_datatables_cob_operators.php?"+vl,
        "type": "GET"
...
*/
      ...
      columns: [
        ...
        { data: "cobrar_operador", className: "number-right",
         ...
         {data: "rec_ope", className:"hidden payok"}
        ],
     select: 
        {
          style:    'os',
...

          {
            ...
            action:function(e,dt,button,config){
              ...
              if($("#date_fim").val()){
                ...
                file_4='';title_4='';}
              config.filename ='cobrancas_operador_'+file_1+''+file_3+''+file_4;
              config.title ='Cobranças Operador: '+title_1+''+title_3+''+title_4;
    ...
    "rowCallback": function(row, data, index){
       ...
       if (data['rec_ope'] =='Sim')
        $(row).find('td:eq(0) .btn-validate').addClass('btn-success').removeClass('btn-warning').attr('title','Remover Validação Cobrança Serviço #' +data['id']);
if (data['rec_ope'] !='Sim')
 $(row).find('td:eq(0) .btn-validate').removeClass('btn-success').addClass('btn-warning').attr('title','Validação Cobrança Serviço #' +data['id']);
...
$('#example tbody').on( 'click', '.btn-warning', function () {
   ...
   idarr=[];
   idarr.push(arr.id);
   ServicesPayment(idarr,arr.cobrar_operador,1);
});
...
$('#example tbody').on( 'click', '.btn-success', function () {
   ...
   idarr=[];
   idarr.push(arr.id);
   ServicesPayment(idarr,arr.cobrar_operador,0);
});
...
if (arr.length >=1){
bootbox.dialog({
  message: "Tem a certeza que pretende <b>"+isvalid+"</b>, "+arr.length+" serviço(s) com ID: <strong>"+txt+"</strong> no Campo Cobrança Operador?<br>O valor total é de: "+vl+"</strong>",
  ...
  buttons: {
    ...
    danger: {
      ...
      $.ajax({ 
      url:'operator/action_close_cob_operators.php',
      ...
      ar = JSON.parse(data);
       if(!ar.success.match(/0/g)){  
         $('.debug-url').html('O(s) '+arr.length+' serviço(s) com ID: <strong>'+txt+'</strong> para estado: '+isvalid+', no campo Cobrança Operador.');
         $("#Modalok").modal();
         $('.received').addClass('disabled');
         txt='';
...
$('#Modalko .debug-url').html('Não tem Cobranças para validar!');
$("#Modalko").modal();
}
}
