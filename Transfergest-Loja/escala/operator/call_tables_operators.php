...
if ($('.cat-txt').text().length < 1){
      ...
      leg = '<span style="font-size: 15px;"><input type="hidden" id="id-op-no" value='+ope+'><input type="hidden" id="id-op" value='+ope+'><input type="hidden" id="id-cat" value='+cat+'> <strong>Categoria: </strong><span class="op-txt">'+n1+'</span> <strong>Operador: </strong><span class="cat-txt">'+n2+'</span><span class="percentagem print-hide"><strong> Ultima % atribuida: </strong>'+pcrtg+'</span></span>';
...
function searchLocalTables(prod,cat){
  ...
  $.ajax({ url:'operator/action_operator.php',
    ...
    success:function(data){
      ...
      header = "<thead class='my-gray'><tr><th class='bdr-w'>Locais</th>"+paxs+"<th class='print-hide' style='text-align: center; min-width:80px;'>Preço Operador</th><th class='print-hide' style='text-align: center; min-width:80px;'>Preço Noturno</th></tr></thead>";
      ...
      for (i=0;i<prods.length;i++){
        ...
        for (j=0;j<prods[i].pvp.length;j++){
          ...
          pn = '<div class="my-medium-gray pn-price"><font class="col-'+j+'-'+prods[i].id+'-pn"><strong>PN: </strong>'+parseFloat(pn_u).toFixed(2)+'€</font><input min="0" data-id="'+prods[i].noturno[j].id+'" class="frm-item form-control editprice-pn-'+prods[i].id+' val-'+j+'-'+prods[i].id+'-pn" value="'+pn_u+'" type="number" step="any"></div>';
...
function StartEditFrmTablePrices2(prod){
$('.editprice-pn-'+prod).removeClass('frm-item').prev().hide();
...
function cancelFrmTableEdit2(prod){
$('#action-prices-pn-'+prod).html("<div style='float: right;' role='group'><button class='btn btn-info btn-sm btn-"+prod+"' onclick='StartEditFrmTablePrices2("+prod+")'><span class='glyphicon glyphicon-edit'></span></button></div>");
  $('#container-calc-'+prod).empty();
  $('.tabela-oper-'+prod).removeClass('w3-pale-green');
  $('.tabela-oper-'+prod+' input').css('margin-top','0');

  $('.editprice-pn-'+prod).addClass('frm-item').prev().show();
  $('#heading-'+prod+' select').val('0');
}
...
function EditTable2(prod){
$('#action-prices-pn-'+prod).html("<div style='float:right;' role='group'><button title='Guardar' class='btn btn-sm btn-success' onclick='EditTableData2("+prod+")'><span class='glyphicon glyphicon-save-file'></span></button><button title='Fechar Edição' class='btn btn-sm btn-default' onclick='cancelFrmTableEdit2("+prod+")'><span class='glyphicon glyphicon-remove-sign'></span></button></div>");
}
...
function EditTableData2(prod){
 dataArray= [];
  $( '.editprice-pn-'+prod ).each(function() {
      !$(this).val() ? $(this).val(0) : false;
        dataArray.push({id:$(this).data('id'),val:$(this).val()});
 });
 updateTablePriceOperator2(dataArray,prod);
}
...
/*EDITAR REGISTOS TABELA OPERADOR*/
function updateTablePriceOperator2(dataA,prod){

$(".back").fadeIn();
  dataValue="arr="+JSON.stringify(dataA)+"&oper="+$('#id-op-no').val()+"&cat="+$('#id-cat').val()+"&action=91";
  $.ajax({ url:'operator/action_operator.php',
    data:dataValue,
    type:'POST',
    cache:false, 
     ...
     $(".back").fadeOut(); 
      if (data.match(/0/g)){
       ...
       $("#mensagem_ko").trigger('click');
        ...
        for( j=0 ; j<dataA.length; j++ ){
          $('.col-'+j+'-'+prod+'-pn').html('<strong>PN: </strong>'+parseFloat($('.val-'+j+'-'+prod+'-pn').val()).toFixed(2)+'€');
        }
      cancelFrmTableEdit2(prod);
