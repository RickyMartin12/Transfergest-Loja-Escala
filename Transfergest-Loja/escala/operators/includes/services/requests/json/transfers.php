...
if ($lang == 'PT')
{
   ...
   $langs['DRIVER_CONT'] = "Manter o voucher para mostrar ao motorista";
}
if ($lang == 'EN')
{
  ...
  if (mysqli_num_rows($result_comp) > 0) {
   ...
   $pdf->Ln(4);
    ...
    $pdf->Ln(5);    
         ...
         $result = mysqli_query($conn, $sql);
          if (mysqli_num_rows($result) > 0) {
            while($row = mysqli_fetch_assoc($result)) {
             ...
             if ($estado == 'Pendente' && $lang == 'EN')
             {
               $estado = 'Pending';
             }
             if ($estado == 'Partida' && $lang == 'EN')
             {
               $estado = 'Departure';
             }
             if ($estado == 'Chegada' && $lang == 'EN')
             {
               $estado = 'Arrival';
             }
             if ($estado == 'Aguarda' && $lang == 'EN')
             {
               $estado = 'Loading';
             }
             if ($estado == 'Confirmado' && $lang == 'EN')
             {
               $estado = 'Confirmed';
             }
             if ($estado == 'Fechado' && $lang == 'EN')
             {
               $estado = 'Close';
             }
             if ($retorno == 'Sim' && $lang == 'EN')
             {
               $retorno = 'Yes';
             }
             if ($servico == 'Airport Transfers' && $lang == 'PT')
             {
               $servico = 'Transferes de Aeroportos';
             }
             if ($servico == 'Parques Temáticos' && $lang == 'EN')
             {
               $servico = 'Theme Parks';
             }
             if ($servico == 'Golf Transfer' && $lang == 'PT')
             {
               $servico = 'Transfer de Golf';
             }
             if ($servico == 'Tours' && $lang == 'PT')
             {
               $servico = 'Viagens';
             }
             if ($servico == 'Transfers' && $lang == 'PT')
             {
               $servico = 'Transferes';
             }
            }
        ...
        $referencia && $referencia !='-/-' ? $pdfhead = $langs['VOUCHER'].''.$referencia : $pdfhead = $langs['VOUCHER'].''.$ids;
        ...
        // Serviço
        $pdf->SetFont('Arial','B',9);
        $pdf->Cell(14.3,6,utf8_decode($langs['SERVICES']),0,0,'L',true);
        $pdf->SetFont('Arial','',9);
        $pdf->Cell(57,6,utf8_decode($servico),0,0,'L',true);
        /* Retorno
        $pdf->SetFont('Arial','B',9);
        $pdf->Cell(15,6,utf8_decode($langs['RETURN']),0,0,'L',true);
        $pdf->SetFont('Arial','',9);
        $pdf->Cell(10,6,utf8_decode($retorno),0,0,'L',true);*/
        // Estado
        $pdf->SetFont('Arial','B',9);
        $pdf->Cell(15,6,utf8_decode(),0,0,'L',true);
        $pdf->SetFont('Arial','',9);
        $pdf->Cell(20,6,utf8_decode(),0,0,'L',true);
        //ID
        $pdf->SetFont('Arial','B',9);
        ...
        // Numero do Quarto
        $pdf->SetFont('Arial','B',9);
        $pdf->Cell(32,6,utf8_decode(),0,0,'L',true);
        $pdf->SetFont('Arial','',9);
        ...
        // Tipo de Pagamento
        $pdf->SetFont('Arial','B',9);
        $pdf->Cell(20,6,utf8_decode($langs['PAY']),0,0,'L',true);
        $pdf->SetFont('Arial','',9);
        $pdf->Cell(30,6,utf8_decode($pag),0,0,'L',true);
        // Valor
        $pdf->SetFont('Arial','B',9);
        $pdf->Cell(94.1,6,utf8_decode($langs['VAL']),0,0,'R',true);
        $pdf->SetFont('Arial','',9);
   ...
   $titulo1 = utf8_decode($c_nome .' | NIF:'.$c_nif.' | RNAVT:'.$c_ravt);
    ...
    $pdf->Cell(0,0,'Pag. '.$pdf->PageNo(),0,0,'R');
