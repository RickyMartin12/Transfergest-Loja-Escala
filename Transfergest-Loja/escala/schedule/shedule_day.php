...
$(this).prepend('<button style="float:right; margin: 3px 3px 0px 0px;" title="Enviar escala do dia para '+$(this).text()+'" class="btn btn-sm btn-default bt-'+$(this).text()+'" onclick="SendServiceSchedule('+nm+')"><span class="glyphicon glyphicon-envelope"></span></button>');
		   ...
		   scaleEndHour: 24,
                ...
                success:function(data){
                 if (data == 0){
                  $('.debug-url').html('Erro, ao atribuir Staff ao Serviço <b>#'+appointment.id+'</b>!<br>A página vai ser reiniciada dentro de 3 segundos.');
                  $("#Modalko").modal();
                 setTimeout(function(){ window.open("/administration/schedule/vehicules_day.php",'_self');}, 2500);
              }   
                },
                error:function(){
                 $('.debug-url').html('Erro, ao atribuir Staff ao Serviço <b>#'+appointment.id+'</b>, p.f. verifique a Acesso Internet.<br>A página vai ser reiniciada dentro de 3 segundos.');
                 $("#Modalko").modal();
                 setTimeout(function(){ window.open("/administration/schedule/shedule_day.php",'_self');}, 2500);
                }
              });
            });   
        });
