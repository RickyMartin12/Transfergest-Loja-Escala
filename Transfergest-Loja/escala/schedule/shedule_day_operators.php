...
$(this).prepend('<button style="float:right; margin: 3px 3px 0px 0px;" title="Enviar escala do dia para '+$(this).text()+'" class="btn btn-sm btn-default bt-'+$(this).text()+'" onclick="SendServiceSchedule('+nm+')"><span class="glyphicon glyphicon-envelope"></span></button>');
		   ...
		   scaleEndHour: 24,
                ...
                success:function(data){
                 ...
                 if (data == 0){
                  $('.debug-url').html('Erro, ao atribuir ao Operador o Transfer <b>#'+appointment.id+'</b>!<br>A página vai ser reiniciada dentro de 3 segundos.');
                  $("#Modalko").modal();
                  setTimeout(function(){ window.open("/administration/schedule/shedule_day_operators.php",'_self');}, 2500);
              }   
                },
                error:function(){
                 $('.debug-url').html('Erro, ao atribuir ao Operador o Transfer <b>#'+appointment.id+'</b>, p.f. verifique a Acesso Internet.<br>A página vai ser reiniciada dentro de 3 segundos.');
                 $("#Modalko").modal();
                 setTimeout(function(){ window.open("/administration/schedule/shedule_day_operators.php",'_self');}, 2500);
                }
              });
...
if (!id || !date){
$('.debug-url').html('Não foi possivel enviar email para o Operador: <b>'+id+'</b>, p.f verifique os dados inseridos no link: "Configuracoes -> Sistema".');
$("#Modalko").modal();
}
...
console.log(data);
       if (data == 0){
        ...
        $('.debug-url').html('Não foi possivel enviar email para o Operador: <b>'+id+'</b>, p.f verifique os dados inseridos no link: "Staff -> Editar".');
        $("#Modalko").modal();
       }
  else if (data == 110){
        $('.debug-url').html('Não foi possivel enviar email, para o Operador: <b>'+id+'</b> sem serviços atribuidos. p.f. atribua servicos.');
        $("#Modalko").modal();
       }
   else if (data == 1111){
        $('.debug-url').html('Os Servicos foram enviados com sucesso, para o operador: <b>'+id+'</b>.');
    ...
    error:function(data){
     $('.debug-url').html('Erro ao enviar dados para o Operador: <b>'+id+'</b>, p.f. verifique o Acesso Internet.');
     $("#Modalko").modal();
    }
  });
