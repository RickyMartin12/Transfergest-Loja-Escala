...
/*
$('.jqx-fill-state-pressed').html("<button id='pdfExport' class='btn btn-default btn-print hidden-xs' title='Imprimir Relatório'><span class='glyphicon glyphicon-print'></span></button>").removeClass('jqx-group-button-normal jqx-button jqx-fill-state-normal jqx-rc-l jqx-rc-r jqx-fill-state-hover jqx-fill-state-pressed');
 $("#pdfExport").click(function () {
alert ('imprimir');
window.print();

var content = $("#scheduler").jqxScheduler('exportData', 'html');
...
/*
                ...
                success:function(data){
                 if (data == 0){
                  $('.debug-url').html('Erro, ao atribuir a viatura ao Transfer <b>#'+appointment.id+'</b>!<br>A página vai ser reiniciada dentro de 3 segundos.<br><a class="btn btn-warning" href="index.html">Atualizar</a>');
                  $("#Modalko").modal();
                 setTimeout(function(){ window.open("/schedule/vehicules_day.php",'_self');}, 2500);
              } 
               },
                error:function(){
                 ...
                 $("#Modalko").modal();
setTimeout(function(){ window.open("/schedule/viaturas_dia.html",'_self');}, 2500);
