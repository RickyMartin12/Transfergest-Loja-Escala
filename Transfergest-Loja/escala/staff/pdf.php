...
$sel_sql=" SELECT * FROM excel WHERE staff = '$user_serv' AND data_servico=$time ORDER BY hrs";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
	    ...
	    $tp_serv = $row['servico'];
        $equipa = "Equipa: ".$row['staff'];
        $voo ="Voo: ".$row['voo'];
        $nome="Nome: ".$row['nome_cl'];
        $local_pick_up = "Recolha: ".$row['local_recolha'];
        $local_destino= "Destino: ".$row['local_destino'];
        $destino_endereco =  "Pt.Ref: ".$row['ponto_referencia'];
        $px = "Nº Px: ".$row['px'];
        $cr = "Nº Cr: ".$row['cr'];
        $bebe = "Nº Bb: ".$row['bebe'];
        $obs = "Obs: ".$row['obs'];
        $companhia = "Operador: ".$row['operador'];
        $ticket = "Ticket: ".$row['ticket'];
        $acobrar = "Cobrar direto: ".$row['cobrar_direto']."€";
