...
case '0':
  $staff = $_POST['st'];
  $sel="SELECT id, cmx_st,local_recolha,local_entrega FROM excel WHERE staff = '$staff' ORDER BY hrs";
...
case '1':
 ...
 $sel="SELECT id, cmx_st, data_servico, hrs FROM excel
 WHERE(data_servico BETWEEN $time_ini_month AND $time_end_month)
 AND staff = '$staff' ORDER BY hrs ";
  ...
  while($row = mysqli_fetch_assoc($result)) {
    ...
    $horaq = date('H:i', $hora);
    $i++;
    $obj[] = array('id'=> $id,'cmx' =>number_format((float)($cmx), 2, '.', ''),'serv'=>(string)$i,'dia' => $diaq,'hora' => $horaq,'vencimento' => number_format((float)($venc_st), 2, '.', ''));
  }

  echo (string)json_encode($obj);
