  ...
  <div class=" my-orange panel panel-default no-print">
    <div class="panel-heading my-orange">
      <div class='row'>
        <div class="col-md-4">
          <h3 class="panel-title" style="color:#000!important;">
           <span class="glyphicon glyphicon-user"></span> Comissões
          </h3>
        </div>
        <div class="col-md-8" style="text-align: right;">
          ...
          <a href="#" class="received btn btn-success disabled" title="Validar Comissão dos serviços como valor Pago ao Staff, campo Comissão Staff"><span class="glyphicon glyphicon-eur"></span><span class="hidden-xs"> Validar</span></a>
        </div>
      </div>
      ...
      <div class='row nosearchprint no-print'>
        ...
        <div class="col-xs-12 cmx_calc hidden">
          <div class="form-group" style='float:right;'>
            <div class="btn-group" data-toggle="buttons">
              <a href="#" class="btn btn-default active" title="Vencimento apenas (V)" onclick='PaymentCalc(1)'> <span class="gly glyphicon glyphicon-user"> </span> (V) </a>
              <a href="#" class="vmc btn btn-info" title="Adicionar Comissão ao Vencimento (C+V)" onclick='PaymentCalc(2)'><span class="gly glyphicon glyphicon-user"></span> (C+V) </a>
              <a href="#"class="cmvmv btn btn-primary" title="Comissão apenas (C)" onclick='PaymentCalc(3)'><span class="gly glyphicon glyphicon-user"></span> (C) </a>

              <a href="#"class="cmo btn btn-success" title="Vencimento+Objectivo (V+O)" onclick='PaymentCalc(4)'><span class="gly glyphicon glyphicon-user"></span> (V+O) </a>
...
function searchValue(vl){
  $("#example, #example_container2, #example_container, .search").css("display","block");
   ...
   "paging": false,
    ...
    "ajax":
      {
        "url" : "team/action_datatables_cobrar_staff.php?"+vl,
        "type": "GET"
...
*/
      ...
      columns: [
        ...
        { data: "id", render: function ( data, type, row ) {
          return '<button title="Validar Comissão Serviço #'+data+'" class="btn-sm btn-validate btn btn-warning" style="margin-left: 5px;">&nbsp;<i class="fa fa-eur"></i>&nbsp; </span></button><button title="Ver todos os detalhes do Serviço #'+data+'" class="btn-sm btn btn-expand btn-info"><i class="fa fa-exclamation-circle"></i> '+data+'</button>';
          }
},
        { data: "referencia" },
        ...
        { data: "cmx_st", className:"number-right",
         ...
         {data: "pag_staf", className:"hidden payok"},
         {data: "v", className:""}
        ],
     select: 
        {
        ...
        buttons: [
          {
            ...
            columns: ':not(:nth-child(6),:nth-child(7),:nth-child(9),:nth-child(10),:nth-child(11),:nth-child(12),:nth-child(13),:nth-child(14),:nth-child(15),:nth-child(16),:nth-child(19),:nth-child(20),:nth-child(23),:nth-child(24),:nth-child(25),:nth-child(27),:nth-child(28),:nth-child(29),:nth-child(30),:nth-child(31),:nth-child(32),:nth-child(33),:nth-child(34),:nth-child(36),:nth-child(37))'
          },
          {
            extend: 'print', 
          ...
          },
          {
            ...
            action:function(e,dt,button,config){
              ...
              config.filename ='comissoes_staff_'+file_1+''+file_3+''+file_4;
...

          {
            ...
            action:function(e,dt,button,config){
              ...
              if($("#date_fim").val()){
                ...
                file_4='';title_4='';}
              config.filename ='comissoes_staff_'+file_1+''+file_3+''+file_4;
              config.title ='Comissões Staff: '+title_1+''+title_3+''+title_4;
...

          {
           ...
           exportOptions: { orthogonal: 'export', columns: ':visible' },
            ...
            "footerCallback": function ( row, data, start, end, display ) {
              ...
              $( api.column( 25 ).footer() ).html('<span class="total_cmx">'+parseFloat(pageTotal.toString()).toFixed(2)+'</span>€<br/><span class="hidden salary_pay" style="width:100px!important;"></span>');                       
    ...
    "rowCallback": function(row, data, index){
        ...
        if (data['v']) vencimento = data['v'];

       if (data['pag_staf'] =='Sim')

        $(row).find('td:eq(0) .btn-validate').addClass('btn-success').removeClass('btn-warning').attr('title','Remover Validação Comissão Serviço #' +data['id']);
if (data['pag_staf'] !='Sim')
 $(row).find('td:eq(0) .btn-validate').removeClass('btn-success').addClass('btn-warning').attr('title','Validação Comissão Serviço #' +data['id']);
             ...
             "columnDefs": [
               ...
               {
                 "targets": [5,6,8,9,10,11,12,13,14,15,18,19,22,23,24,26,27,28,29,30,31,32,33,36],
                 "visible": false,
                 "searchable": false,
                }
             ],
...
$('#example tbody').on( 'click', '.btn-warning', function () {
   ...
   idarr=[];
   idarr.push(arr.id);
   ServicesPayment(idarr,arr.cmx_st,1);
});
...
$('#example tbody').on( 'click', '.btn-success', function () {
   ...
   idarr=[];
   idarr.push(arr.id);
   ServicesPayment(idarr,arr.cmx_st,0);
});
...
function PaymentCalc(i){
total_is = 0;
cmx = $('.total_cmx').text();

$('.salary_pay').removeClass('hidden');
switch (i){ 
case 1:
$('.total_cmx').addClass('linetr');
$('.salary_pay').html("Venc.: "+parseFloat(vencimento).toFixed(2)+"€");
$("html, body").animate({ scrollTop: $(document).height() }, 1000);break;

case 2:
$('.total_cmx').removeClass('linetr');
total_is=parseFloat(vencimento)+parseFloat(cmx);
$('.salary_pay').html("Venc.+Cmx : "+total_is.toFixed(2)+"€");
$("html, body").animate({ scrollTop: $(document).height() }, 1000);break;

case 3:
$('.total_cmx').removeClass('linetr');
$('.salary_pay').addClass('hidden');
$("html, body").animate({ scrollTop: $(document).height() }, 1000);break;
...
$('.salary_pay').removeClass('hidden');
$('.total_cmx').removeClass('linetr');
total_dia=parseFloat(cmx);
total_venc=parseFloat(vencimento);
if (total_dia>total_venc){
$('.salary_pay').html("Venc.: "+total_dia.toFixed(2)+"€");
$("html, body").animate({ scrollTop: $(document).height() }, 1000);
}

else {
$('.salary_pay').html("Venc.: "+total_venc.toFixed(2)+"€");
$("html, body").animate({ scrollTop: $(document).height() }, 1000);
}break;
...
if (arr.length >=1){
bootbox.dialog({
  message: "Tem a certeza que pretende <b>"+isvalid+"</b>, "+arr.length+" serviço(s) com ID: <strong>"+txt+"</strong> no Campo Comissão Staff?<br>O valor total é de: "+vl+"</strong>",
  ...
  buttons: {
    ...
    danger: {
      ...
      $.ajax({ 
      url:'team/action_close_cmx_staff.php',
      ...
      ar = JSON.parse(data);
       if(!ar.success.match(/0/g)){  
         $('.debug-url').html('O(s) '+arr.length+' serviço(s) com ID: <strong>'+txt+'</strong> para estado: '+isvalid+', no campo Comissão Staff.');
         $("#Modalok").modal();
         $('.received').addClass('disabled');
         txt='';
...
$('#Modalko .debug-url').html('Não tem Comissões para validar!');
$("#Modalko").modal();
}
}
