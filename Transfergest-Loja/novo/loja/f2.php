...
<input type="text" required class="form-control" id="arr_fl_nr" name="arr_fl_nr"/>
 ...
 <div class="col-lg-3 col-md-6 col-xs-12 form-group">
  ...
  <input type='hidden' id="arr_fl_tm2">
  <input type='hidden' id="arr_fl_dt2">
  </div>
 </div>
...
$("#data_reserva_hotel").val(date_r);
      ...
      {
        var defaultBounds4 = new google.maps.LatLngBounds(
        ...
        autocomplete4 = new google.maps.places.Autocomplete(
            ...
            {bounds: defaultBounds4,componentRestrictions: {country: 'pt'}},
            {types: ['geocode']});
...
flag_dt = 0;
  ...
  $(function () {
    ...
    $("input[name='arr_fl_dt']").val(date_r);
      ...
      var data_hora = $("input[name='arr_fl_dt']").val();
...
return;
  ...
  $('.load').show();
  dataValue=$( this ).serialize()+"&prod="+id_prods+"&label="+$('#all-classes').val()+"&action=10&oneway="+$('input[type=radio]:checked').val()+"&promo="+$('#promo-code').val()+"&from="+$('#all-places').val()+"&to="+$('#match-places').val()+"&pax="+$('#all-classes option:selected').text()+"&pp="+pp;
$.ajax({
      type: "POST",
...
bootbox.dialog({
  ...
  buttons: {
    ...
    danger: {
      ...
      $('body').css('padding-right','0');
      $('.load').show();
      dataValue=$('.twowayairport').serialize()+'&dominio='+$('.dominio').val()+'&navigator='+$("#navigator").val()+'&categoria='+$("#cat").val()+'&url='+$(".url").val()+'&coord4='+$("#coord").val()+'&action=1';
       $.ajax({
        type: "POST",
        url: url_forms+'form2_t.php',
        data: dataValue,
        success: function(data){
         arr=[];
         arr =  JSON.parse(data);
