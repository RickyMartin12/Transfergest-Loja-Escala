...
return;
  ...
  $('.load').show();
  dataValue=$( this ).serialize()+"&prod="+id_prods+"&label="+$('#all-classes').val()+"&action=11&oneway="+$('input[type=radio]:checked').val()+"&promo="+$('#promo-code').val()+"&from="+$('#all-places').val()+"&to="+$('#match-places').val()+"&pax="+$('#all-classes option:selected').text()+"&pp="+pp;
$.ajax({
      type: "POST",
...
bootbox.dialog({
  ...
  buttons: {
    ...
    danger: {
      ...
      $('body').css('padding-right','0');
      $('.load').show();
      dataValue=$('.twowayairport').serialize()+'&dominio='+$('.dominio').val()+'&navigator='+$("#navigator").val()+'&categoria='+$("#cat").val()+'&url='+$(".url").val()+'&coord3='+$("#coord").val()+'&pickup_recommended='+$("#dep_fl_pick").val()+'&action=1';
       $.ajax({
        type: "POST",
        url: url_forms+'form3_t.php',
        data: dataValue,
        success: function(data){
         arr=[];
         arr =  JSON.parse(data);
