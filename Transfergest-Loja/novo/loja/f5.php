...
<div class="col-lg-5 col-md-4 col-xs-12 form-group">
...
<input type="text" class="form-control" id="dep_fl_addr2" name="dep_fl_addr2"/>
<input type="hidden" id="coord_3">
...
<div class="col-lg-12">
<div class="panel panel-default">
<div class="panel-heading"><h3 class="panel-title TRANSFER_DEL_DETAILS"></h3></div>
<div class="panel-body">

<div class="col-lg-2 col-md-4 col-xs-12 form-group">
 <font class='TRANSFER_DEL_DT'> *</font>
  <div class='input-group date' id="dep_fl_dt">
  <input type='text' required name="dep_fl_dt" class="form-control TRANSFER_DEL_DT_LABEL"/><span class="btn-primary input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
  </div>
 </div>
...
<div class="col-lg-5 col-md-4 col-xs-12 form-group">
<font class='TRANSFER_DEL_ADDR'></font>
<input type="text" class="form-control" id="dep_fl_addr" name="dep_fl_addr" readonly/>
...
<div class="col-lg-5 col-md-4 col-xs-12 form-group">
<font class='TRANSFER_PICK_DEP'></font>
<input type="text" class="form-control" id="dep_fl_addr3" name="dep_fl_addr3" readonly/>
<input type="hidden" id="coord_4">
...
$("#arr_fl_addr").blur(function()
{
   var ad = $("#arr_fl_addr").val();
   $("#dep_fl_addr3").val(ad);
});


$("#dep_fl_addr2").blur(function()
{
   var ad1 = $("#dep_fl_addr2").val();
   $("#dep_fl_addr").val(ad1);
});


$("#arr_fl_addr").on('input', function()
{
   var ad = $("#arr_fl_addr").val();
   $("#dep_fl_addr3").val(ad);
});
...
$("#dep_fl_addr2").on('input', function()
{
   var ad1 = $("#dep_fl_addr2").val();
   $("#dep_fl_addr").val(ad1);
});

$("#arr_fl_addr").click(function()
{
   var ad = $("#arr_fl_addr").val();
   $("#dep_fl_addr3").val(ad);
});
...
flag_dt = 0;
      ...
      function initAutocomplete4() 
      {
        ...
        autocomplete4 = new google.maps.places.Autocomplete(
            (document.getElementById('dep_fl_addr2')),
  ...
  $(function () {
        ...
        $("input[name='arr_fl_dt']").val(date_str);

        $("input[name='dep_fl_dt']").val(date_str);
   ...
   $('#arr_fl_dt').datetimepicker({useCurrent: false, ignoreReadonly: true, locale: $('.LANGUAGE').val(), sideBySide: true, minDate: d});
        
    $('#dep_fl_dt').datetimepicker({useCurrent: false, ignoreReadonly: true, minDate: d, locale: $('.LANGUAGE').val(), sideBySide: true});

   $("#arr_fl_dt").on("dp.change", function (e) {$('#dep_fl_dt').data("DateTimePicker").minDate(e.date); $('#arr_fl_dt').data("DateTimePicker").maxDate(e.date);});

   $("#dep_fl_dt").on("dp.change", function (e) {$('#arr_fl_dt').data("DateTimePicker").maxDate(e.date); $('#arr_fl_dt').data("DateTimePicker").maxDate(e.date);});
...
return;
  ...
  $('.load').show();
  dataValue=$( this ).serialize()+"&prod="+id_prods+"&label="+$('#all-classes').val()+"&action=13&oneway="+$('input[type=radio]:checked').val()+"&promo="+$('#promo-code').val()+"&from="+$('#all-places').val()+"&to="+$('#match-places').val()+"&pax="+$('#all-classes option:selected').text()+"&pp="+pp;
$.ajax({
      type: "POST",
...
bootbox.dialog({
  ...
  buttons: {
    ...
    danger: {
      ...
      $('body').css('padding-right','0');
      $('.load').show();
      dataValue=$('.twowayairport').serialize()+'&dominio='+$('.dominio').val()+'&navigator='+$("#navigator").val()+'&categoria='+$("#cat").val()+'&url='+$(".url").val()+'&coord1='+$("#coord_1").val()+'&coord3='+$("#coord_3").val()+'&action=1';
       $.ajax({
        type: "POST",
        url: url_forms+'form5_t.php',
        data: dataValue,
        success: function(data){
         arr=[];
         arr =  JSON.parse(data);
